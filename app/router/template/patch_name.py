...
def __usecase_di(db:Session=Depends(get_db)):
    return TemplateUpdateNameUseCase(db,TemplateRepository(db))

@router.patch("/{template_id}", response_model=TemplateDisplay)
async def template_patch_name(group_id: str,template_id:str,request:TemplateCreateBase, usecase:TemplateUpdateNameUseCase=Depends(__usecase_di)):

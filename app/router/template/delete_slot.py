...
from app.schemas.template import TemplateDisplay, TemplateTaskBase

router = APIRouter()

def __usecase_di(db:Session=Depends(get_db)):
    return TemplateDeleteSlotUseCase(db,TemplateRepository(db))

@router.delete("/{template_id}/slots/", response_model=TemplateDisplay)
async def template_patch_slots(group_id: str,template_id:str,request:TemplateTaskBase, 
